...
// Copyright (c) 2015-2017 The ANT developers
...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::ANT)
    {
    }
    ...
    if (!fLiteMode) {
...
#else
       {
        if (fMasterNode) {
            ...
            if (!fEnableDarksend) {
                ui->toggleDarksend->setText(tr("Start Ant7send"));
            } else {
                ui->toggleDarksend->setText(tr("Stop Ant7send"));
            }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("ANT")
    updateDisplayUnit();
}
...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeAnt7Amount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeAnt7Amount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeAnt7Amount = strAnonymizeAnt7Amount.remove(strAnonymizeAnt7Amount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAnt7Amount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeAnt7Amount * COIN) nMaxToAnonymize = nAnonymizeAnt7Amount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeAnt7Amount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeAnt7Amount));
        strAnonymizeAnt7Amount = strAnonymizeAnt7Amount.remove(strAnonymizeAnt7Amount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAnt7Amount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
